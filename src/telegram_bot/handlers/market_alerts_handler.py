    ...
    try:
        ...
        if action == "toggle":
            ...
            if alert_type in user_subscriptions:
                ...
                if success:
                    await query.answer(
                        f"Вы отписались от уведомлений: {ALERT_TYPES.get(alert_type, alert_type)}",
                    )
                else:
                ...
                if success:
                    await query.answer(
                        f"Вы подписались на уведомления: {ALERT_TYPES.get(alert_type, alert_type)}",
                    )
                else:
            ...
            if success:
                ...
                await query.answer(
                    "Возникли ошибки при подписке на некоторые типы уведомлений",
                )
