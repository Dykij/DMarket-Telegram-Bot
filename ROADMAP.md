...
**Статус**: 🔴 **КРИТИЧНО** - Ключевой модуль (1780 строк)
...
```
src/dmarket/scanner/
├── __init__.py           # Экспорт ArbitrageScanner
├── scanner.py            # Основной класс сканера (~400 строк)
├── levels.py             # Конфигурации уровней (boost, standard, etc.) (~200 строк)
├── cache.py              # Cache management (~200 строк)
├── filters.py            # Item filtering logic (~300 строк)
├── analysis.py           # Profit calculation, statistics (~300 строк)
└── multi_game.py         # Multi-game support (~200 строк)
```

**Критерий завершения**: ArbitrageScanner работает как прежде, тесты проходят
