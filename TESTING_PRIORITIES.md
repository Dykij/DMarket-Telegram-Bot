...
> **Дата обновления:** 20 декабря 2025 г. (обновлено)
> **Текущее покрытие:** 60.09% ✅ (цель 60%+ достигнута!)
> **Всего тестов:** 2356+ (все проходят)
> **Необходимо добавить:** ~520 тестов для достижения 70%
...
| **Аутентификация** | `src/dmarket/api/auth.py`        | 55    | ⚡ ВЫСОКИЙ | 15+ тестов |
| **Кэш**            | `src/dmarket/api/cache.py`       | 46    | ⚡ СРЕДНИЙ | 20+ тестов |
| **Инвентарь**      | `src/dmarket/api/inventory.py`   | 44    | ⚡ СРЕДНИЙ | 15+ тестов |

**Итого:** 805 строк без покрытия, ~190 тестов необходимо

#### Детальный план тестирования API модулей
...
### 2. Arbitrage Module (Приоритет: 🔥 КРИТИЧЕСКИЙ)

| Модуль       | Файл                       | Строк | Приоритет     | Тестов     |
| ------------ | -------------------------- | ----- | ------------- | ---------- |
| **Арбитраж** | `src/dmarket/arbitrage.py` | 553   | 🔥 КРИТИЧЕСКИЙ | 60+ тестов |
...
### Неделя 1-2: DMarket API (🔥 КРИТИЧЕСКИЙ)

**Цель:** Покрыть 8 модулей API тестами (~190 тестов)

- [ ] **client.py** - 40 тестов (HMAC auth, HTTP requests, rate limiting, retry logic)
- [ ] **wallet.py** - 25 тестов (balance, transactions, deposits, withdrawals)
- [ ] **market.py** - 30 тестов (items, filters, pagination)
- [ ] **trading.py** - 25 тестов (buy, sell, offers, cancellations)
- [ ] **targets_api.py** - 20 тестов (create, delete, update targets)
- [ ] **auth.py** - 15 тестов (authentication flow)
- [ ] **cache.py** - 20 тестов (caching logic, TTL, eviction)
- [ ] **inventory.py** - 15 тестов (inventory operations)

**Ожидаемый результат:** 0% → 75%+ покрытие API модулей, +4-5% общего покрытия

- [ ] **cache.py** - 20+ тестов (caching logic)
- [ ] **inventory.py** - 15+ тестов (inventory operations)

**Ожидаемый результат:** +4-5% общего покрытия

---

### Неделя 3-4: Arbitrage Module (🔥 КРИТИЧЕСКИЙ)

**Цель:** Довести arbitrage.py до 80%+ покрытия (~60 тестов)

- [ ] **arbitrage.py** - 60 тестов
  - Инициализация сканера (5 тестов)
  - Сканирование уровней: boost, standard, medium, advanced, pro (10 тестов)
  - Расчет прибыли с различными комиссиями (8 тестов)
  - Фильтрация по прибыли/цене/игре/ликвидности (10 тестов)
  - Сортировка результатов (5 тестов)
  - Валидация opportunities (8 тестов)
  - Property-based тесты (Hypothesis) - инварианты прибыли (9 тестов)
  - Edge cases: нулевые цены, отрицательная прибыль, пустые результаты (5 тестов)

**Ожидаемый результат:** 0% → 80%+ покрытие arbitrage.py, +2-3% общего покрытия

---

### Неделя 5-6: Telegram Handlers (⚡ ВЫСОКИЙ)
...
- [ ] **notification_digest_handler.py** - 40 тестов
  - Создание дайджестов (8 тестов)
  - Группировка уведомлений (8 тестов)
  - Форматирование (8 тестов)
  - Отправка (8 тестов)
  - Edge cases (8 тестов)
