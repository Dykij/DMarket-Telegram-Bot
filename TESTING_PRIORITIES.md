...
| Файл/Папка | Причина | Связанные тесты |
|------------|---------|-----------------|
| `src/dmarket/dmarket_api.py` | Устаревший, дублирует `api/client.py` | `tests/dmarket/test_dmarket_api.py`, `test_dmarket_api_basic.py` |
...
**ВАЖНО:** Перед удалением модулей:
1. Проверить что функционал перенесен в другие модули
2. Удалить связанные тесты
3. Обновить импорты во всех файлах
4. Запустить полный тестовый набор

```bash
# Пример команды для проверки использования модуля
...
| Модуль | Строк кода | Тестов | Примечание |
|--------|-----------|--------|------------|
