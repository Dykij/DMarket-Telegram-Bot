...
async def test_hierarchical_cache_market_items(hierarchical_cache):
    ...
    # Set
    result = await hierarchical_cache.set_market_items(
        data, "csgo", "standard", 100, 1000
    )
    assert result is True
    
    # Get
    cached = await hierarchical_cache.get_market_items(
        "csgo", "standard", 100, 1000
    )
    assert cached == data
...
async def test_hierarchical_cache_invalidate_market_data(hierarchical_cache):
    ...
    await hierarchical_cache.set_market_items(
        {"test": "data"}, "csgo", "standard"
    )
