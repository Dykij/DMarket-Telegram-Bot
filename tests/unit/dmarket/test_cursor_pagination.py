...
class TestCursorPagination:
    ...
    async def test_cursor_pagination_enabled_by_default(self, mocker):
    ...
    async def test_cursor_pagination_multiple_pages(self, mocker):
        ...
        mock_request = mocker.patch.object(api, "_request", side_effect=responses)
    ...
    async def test_cursor_pagination_respects_max_items(self, mocker):
    ...
    async def test_cursor_pagination_stops_on_empty_cursor(self, mocker):
    ...
    async def test_offset_pagination_fallback(self, mocker):
        """Test that offset pagination works when use_cursor=False."""
        api = DMarketAPI(public_key="test", secret_key="test")

        # Mock get_market_items (used by offset pagination)
        mock_get_items = mocker.patch.object(api, "get_market_items")
    ...
    async def test_cursor_pagination_handles_nextCursor_field(self, mocker):
    ...
    async def test_cursor_pagination_with_filters(self, mocker):
    ...
    async def test_cursor_pagination_with_empty_response(self, mocker):
    ...
    async def test_cursor_pagination_cursor_values(self, mocker, cursor_value):
