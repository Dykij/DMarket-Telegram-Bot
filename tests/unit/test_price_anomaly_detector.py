...
class TestFindPriceAnomalies:
    ...
    async def test_returns_empty_list_on_no_items(self, mock_api_client):
        """Test returns empty list when no items available."""
        mock_api_client.get_market_items = AsyncMock(return_value={"items": []})
...
class TestCompareItemPrices:
    ...
    def test_compares_all_price_pairs(self):
        """Test compares all price pairs in list."""
        items_list = [
    ...
    def test_skips_unprofitable_pairs(self):
        """Test skips pairs that aren't profitable."""
        items_list = [
            {"price": 10.0, "item": {"title": "Test"}},
