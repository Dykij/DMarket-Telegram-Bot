...
class TestAddPriceAlert:
    ...
    async def test_add_price_alert_basic(self, mock_storage):
        ...
        with patch('src.telegram_bot.notifications.alerts.get_storage', return_value=mock_storage):
...
class TestGetUserAlerts:
    ...
    async def test_get_user_alerts_returns_list(self, mock_storage_with_alerts):
        ...
        with patch('src.telegram_bot.notifications.alerts.get_storage', return_value=mock_storage_with_alerts):
    ...
    @pytest.mark.asyncio
    async def test_get_user_alerts_contains_correct_data(self, mock_storage_with_alerts):
        """Test that alerts contain expected data."""
        with patch('src.telegram_bot.notifications.alerts.get_storage', return_value=mock_storage_with_alerts):
            alerts = await get_user_alerts(user_id=12345)
            
            # Find the first alert
            ak47_alert = next((a for a in alerts if "AK-47" in a.get("title", "")), None)
...
class TestEdgeCases:
    ...
    async def test_get_alerts_for_different_users(self, mock_storage_with_alerts):
        ...
        storage2 = MagicMock()
        storage2.get_user_data = MagicMock(return_value={"alerts": [{"id": "2"}, {"id": "3"}]})
        
        with patch('src.telegram_bot.notifications.alerts.get_storage', return_value=storage1):
            alerts1 = await get_user_alerts(user_id=111)
            assert len(alerts1) == 1
        
        with patch('src.telegram_bot.notifications.alerts.get_storage', return_value=storage2):
