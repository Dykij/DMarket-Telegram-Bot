...
### Фаза 1: Knowledge Base System (Приоритет: Высокий)

**Задачи**:
1. [ ] Создать базовую структуру `KnowledgeEntry` и `KnowledgeBase`
2. [ ] Интегрировать с базой данных (PostgreSQL)
3. [ ] Добавить автоматическое обучение после сделок
4. [ ] Интегрировать с `ArbitrageScanner` для персонализации
5. [ ] Добавить Telegram команды для просмотра накопленных знаний
6. [ ] Написать тесты

**Файлы для создания/изменения**:
- `src/utils/knowledge_base.py` (новый)
- `src/models/knowledge.py` (новый)
- `src/dmarket/arbitrage_scanner.py` (изменение)
- `src/telegram_bot/handlers/knowledge_handler.py` (новый)
