# 🚀 Быстрый старт CI/CD

**Дата**: 23 ноября 2025 г.

---

## ✅ Что уже настроено

### GitHub Actions Workflows

1. **CI Pipeline** (`.github/workflows/ci.yml`)
   - ✅ Автоматический запуск при push/PR
   - ✅ Линтинг (Ruff)
   - ✅ Форматирование (Ruff)
   - ✅ Проверка типов (MyPy)
   - ✅ Тесты на Python 3.10, 3.11, 3.12
   - ✅ Security scan (Bandit, Safety)

2. **Code Quality** (`.github/workflows/quality.yml`)
   - ✅ Детальная проверка качества кода
   - ✅ Анализ сложности (Radon, Xenon)
   - ✅ Автоматические комментарии на PR

3. **Coverage** (`.github/workflows/coverage.yml`)
   - ✅ Генерация отчетов покрытия
   - ✅ Интеграция с Codecov
   - ✅ Coverage diff для PR

4. **Release** (`.github/workflows/release.yml`)
   - ✅ Автоматический релиз при создании тега
   - ✅ Сборка Docker образов (multi-platform)
   - ✅ Публикация в GitHub Container Registry
   - ✅ Создание GitHub Release с changelog

5. **Dependencies** (`.github/workflows/dependencies.yml`)
   - ✅ Еженедельная проверка обновлений
   - ✅ Автоматическое создание PR с обновлениями

---

## 📦 Первоначальная настройка

### 1. Активация Pre-commit Hooks

```bash
# Установить pre-commit
pip install pre-commit

# Активировать хуки
pre-commit install

# Первый запуск (проверит все файлы)
pre-commit run --all-files
```

### 2. Настройка Codecov (опционально)

1. Зайдите на [codecov.io](https://codecov.io)
2. Войдите через GitHub
3. Выберите репозиторий `DMarket-Telegram-Bot`
4. Скопируйте токен
5. Добавьте в GitHub Secrets:
   - Настройки → Secrets and variables → Actions
   - New repository secret
   - Name: `CODECOV_TOKEN`
   - Value: `<ваш токен>`

### 3. Проверка работы CI

```bash
# Создайте тестовую ветку
git checkout -b test/ci-setup

# Сделайте небольшое изменение
echo "# CI/CD Test" >> test_ci.md

# Коммит и push
git add test_ci.md
git commit -m "test: verify CI pipeline"
git push -u origin test/ci-setup

# Создайте PR на GitHub
# CI автоматически запустится
```

---

## 🔄 Ежедневное использование

### Перед коммитом

```bash
# 1. Проверка и автоисправление
ruff check . --fix
ruff format .

# 2. Проверка типов
mypy src/

# 3. Запуск тестов
pytest tests/ --cov=src

# 4. Если всё ОК - коммит
git add .
git commit -m "feat: add new feature"

# Pre-commit автоматически запустится
```

### При создании PR

1. **Создайте PR на GitHub**
2. **Дождитесь CI** (обычно 3-5 минут)
3. **Проверьте результаты**:
   - ✅ Все чеки должны быть зелеными
   - 📊 Проверьте coverage отчет
   - 💬 Прочитайте автоматические комментарии

### Если CI упал

```bash
# 1. Посмотрите логи в GitHub Actions
# 2. Исправьте проблему локально
# 3. Push изменений

git add .
git commit -m "fix: resolve CI issues"
git push

# CI автоматически перезапустится
```

---

## 🎯 Создание релиза

### Вариант 1: Через тег

```bash
# 1. Обновите версию в файлах
# pyproject.toml, src/__init__.py и т.д.

# 2. Коммит изменений
git add .
git commit -m "chore: bump version to 1.0.0"
git push

# 3. Создайте и запушьте тег
git tag v1.0.0
git push origin v1.0.0

# Release workflow запустится автоматически
```

### Вариант 2: Через GitHub UI

1. Перейдите в **Actions** → **Release**
2. Нажмите **Run workflow**
3. Укажите версию (например, `1.0.0`)
4. Нажмите **Run workflow**

### После релиза

Автоматически создастся:

- ✅ GitHub Release с changelog
- ✅ Docker образ в ghcr.io
- ✅ Artifacts (wheels, source distribution)

---

## 📊 Мониторинг CI/CD

### GitHub Actions

- **Статус**: <https://github.com/Dykij/DMarket-Telegram-Bot/actions>
- **Workflows**: Все workflows в `.github/workflows/`
- **Artifacts**: Скачать можно после завершения workflow

### Codecov

- **Dashboard**: <https://codecov.io/gh/Dykij/DMarket-Telegram-Bot>
- **Coverage Trends**: График изменения покрытия
- **File Browser**: Детальное покрытие по файлам

### Docker Images

```bash
# Список всех образов
docker images ghcr.io/dykij/dmarket-telegram-bot

# Pull конкретной версии
docker pull ghcr.io/dykij/dmarket-telegram-bot:1.0.0
```

---

## 🛠️ Частые проблемы

### Pre-commit слишком долго работает

```bash
# Пропустить pre-commit для конкретного коммита
git commit -m "message" --no-verify

# ⚠️ Используйте редко, только в крайних случаях!
```

### CI падает на MyPy

```bash
# Запустить MyPy локально с теми же настройками
mypy src/ --config-file=pyproject.toml

# Исправить ошибки типов
# Или добавить # type: ignore если нужно
```

### Coverage слишком низкий

```bash
# Посмотреть детальный отчет
pytest tests/ --cov=src --cov-report=html
open htmlcov/index.html

# Найти непокрытые строки
coverage report --show-missing

# Написать тесты для непокрытого кода
```

### Docker build падает

```bash
# Тестировать локально
docker build -t dmarket-bot:test .

# Проверить логи
docker logs <container-id>

# Проверить Dockerfile
docker build --no-cache -t dmarket-bot:test .
```

---

## 📚 Полезные команды

### Проверка перед коммитом

```bash
# Полная локальная проверка (как в CI)
make qa

# Или по частям:
make lint      # Только линтинг
make format    # Только форматирование
make typecheck # Только проверка типов
make test      # Только тесты
```

### Работа с зависимостями

```bash
# Обновить зависимости
pip install --upgrade -r requirements.txt

# Проверить устаревшие пакеты
pip list --outdated

# Проверить уязвимости
pip-audit
safety check
```

### Работа с Docker

```bash
# Локальная сборка
docker-compose build

# Запуск
docker-compose up -d

# Логи
docker-compose logs -f bot

# Остановка
docker-compose down
```

---

## 🎓 Дополнительные ресурсы

- 📖 [Полная документация CI/CD](CI_CD_GUIDE.md)
- 🔧 [Code Quality Guide](code_quality_tools_guide.md)
- 🧪 [Testing Guide](testing_guide.md)
- 🐳 [Deployment Guide](deployment.md)

---

## ✅ Checklist для contributors

- [ ] Pre-commit hooks установлены
- [ ] Все тесты проходят локально
- [ ] Coverage >= 80%
- [ ] MyPy проверка пройдена
- [ ] Ruff линтинг и форматирование пройдены
- [ ] Нет security warnings
- [ ] PR создан с описанием изменений
- [ ] CI проверки прошли успешно

---

**Готово к использованию!** 🎉

Все workflows настроены и готовы к работе. При следующем push в `main` или создании PR CI автоматически запустится.
